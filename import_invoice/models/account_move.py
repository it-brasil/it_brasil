...
class AccountMove(models.Model):
    ...
    state = fields.Selection(selection=[('imported', 'Importado')])
    ...
    def get_partner_nfe(self, nfe, destinatary):
    ...
    def import_nfe(self, company_id, nfe, nfe_xml):
        invoice_dict = {}
        if self.existing_invoice(nfe):
            raise UserError('Documento Eletrônico já importado!')

        partner_vals = self._get_company_invoice(nfe)
        ...
        partner = self.get_partner_nfe(
            nfe, partner_vals['destinatary']) 
        invoice_dict.update(partner)
        invoice_dict.update(self.get_ICMSTot(nfe))
        invoice_dict.update(self.get_items( nfe, company_id, partner['partner_id'], invoice_dict['partner_id']))
    ...
    def generate_eletronic_document(self, xml_nfe, create_partner):
        ...
        invoice_dict.update(self.get_compra(nfe))
        invoice_dict.pop('destinatary', False)
        return self.create(invoice_dict)
